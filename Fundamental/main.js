...
client.on("interactionCreate",async (interaction, message) => {
  ...
  if(interaction.isButton()) {
     let etkinlikcik = "1007771326381051904"
     let cekiliscik = "1007771325466693722"
     let turnuva = "1007771327312187464"
...
client.on("interactionCreate", async interaction => {
  ...
  if (menu === "renks") {
    let color = new Map([
      ["kirmizi", "1007771260303986778"],
      ["turuncu", "1007771257246331121"],
      ["mor", "1007771259372847274"],
      ["kahverengi", "1007771255954485270"],
      ["yesil", "1007771258290700368"],
      ["sari", "1007771261264478249"],


    ])
    let role = color.get(interaction.values[0])
    let renkroller = ["1007771260303986778", "1007771257246331121", "1007771259372847274", "1007771255954485270", "1007771258290700368", "1007771261264478249"]
  ...
  if (menu === "renks") {
    let color = new Map([
      ["Balık", "1003104143931605022"],
      ["Kova", "1003104143931605021"],
      ["Oğlak", "1003104143931605020"],
      ["Yay", "1003104143931605019"],
      ["İkizler", "1003104143931605018"],
      ["Boğa", "1003104143931605017"],
      ["Koç", "1003104143931605016"],
      ["Yengeç", "1003104143931605015"],
      ["Aslan", "1003104143931605014"],
      ["Başak", "1003104143931605013"],
      ["Terazi", "1003104143919038483"],
      ["Akrep", "1003104143919038482"],


    ])
    let role = color.get(interaction.values[0])
    let renkroller = ["1003104143931605022", "1003104143931605021", "1003104143931605020", "1003104143931605019", "1003104143931605018", "1003104143931605017", "1003104143931605016", "1003104143931605015", "1003104143931605014", "1003104143931605013", "1003104143919038483", "1003104143919038482"] 
    if (!member.roles.cache.has(data.TEAM) && !member.roles.cache.has(data.BOOST) && !member.permissions.has("ADMINISTRATOR")) {
      interaction.reply({ content: `Bu sistemden sadece isminde \`${data.TAG}\` bulunan ve Booster üyelerimiz yararlanabilmektedir.`, ephemeral: true })
  } else {
...
client.on("message", async message => {
  ...
  if (message.member.roles.cache.some(rol => komut.roller.includes(rol.id)) || komut.kisiler.includes(message.author.id) || data2.some(veri => komut.sorumluluk.includes(veri)) || message.member.permissions.has("ADMINISTRATOR")) {

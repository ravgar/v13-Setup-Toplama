...
module.exports.run = async (client, message, args, durum) => {
    ...
    message.reply({embeds: [new MessageEmbed().setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true}),).setDescription(`${client.emojis.cache.find(x => x.name == "wex_afk")} Hey ${message.author}, \`${reason}\` sebebiyle afk modune geçiş yaptın.`)]}).then(e => setTimeout(() => e.delete().catch(() => { }), 15000))
    let newData = afk({

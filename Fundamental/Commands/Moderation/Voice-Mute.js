...
module.exports.run = async (client, message, args, durum, kanal) => {
   ...
   if (sec == "setup") {
    ...
    if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has("ADMINISTRATOR") || message.author.id === message.guild.owner.id) {
        ...
        if(!message.member.permissions.has("8") && !message.member.roles.cache.has(server.EnAltYetkiliRol) && !server.VMUTEAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "wex_cancel")}`)
                ...
                Puan: 0
                    ...
                    if (muteSorumlusu.length >= 1 && client.channels.cache.get(muteLogKanal) && muteLimit >= 1) {
                        ...
                        let embedcik = new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({
                            dynamic: true
                        })).setDescription(`${client.emojis.cache.find(res => res.name === "wex_vmute")} ${target} (**${target.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle geçiçi olarak susturuldu. (Ceza Numarası: \`#${cezaID+1}\`)`)

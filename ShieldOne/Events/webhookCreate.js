...
.setTimestamp()
.setDescription(`${entry.executor} (**${entry.executor.id}**) üyesi __**${moment(Date.now()).format('LLL')}**__ tarihinde \`${channel.name}\` (**${channel.id}**) kanalında bir webhook **oluşturdu.**\`\`\`js
Üye sunucudan atıldı sildiği oluşturduğu webhook silindi.\`\`\``)

...
let entry = await guild.guild.fetchAuditLogs({type: 'MEMBER_PRUNE'}).then(audit => audit.entries.first())
...
.setTimestamp()
.setDescription(`${entry.executor} (**${entry.executor.id}**) üyesi __**${moment(Date.now()).format('LLL')}**__ tarihinde üye çıkarma işlemini **uyguladı.**\`\`\`js

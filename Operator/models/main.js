...
client.on("clickMenu", async menu => {
  if (menu.values[0] === 'profile') {
    await menu.reply.think(true);
    let member = menu.guild.members.cache.get(menu.clicker.member.id);
    let nickname = menu.clicker.member.displayName == menu.clicker.member.username ? "" + menu.clicker.member.username + " [Yok] " : menu.clicker.member.displayName
    const roles = member.roles.cache.filter(role => role.id !== menu.guild.id).sort((a, b) => b.position - a.position).map(role => `<@&${role.id}>`);
    const rolleri = [];
    if (roles.length > 6) {
    ...
    let profilembed = new Discord.MessageEmbed().setAuthor(menu.guild.name, menu.guild.iconURL({
      dynamic: true
  })).setColor("RANDOM").setDescription(`Selam ${menu.clicker.member}, ${menu.guild.name} sunucusundaki kullanıcı bilgilerin aşağıda belirtilmiştir..\n\n**❯ Kullanıcı Bilgisi**\n\`•\` Hesap: ${menu.clicker.member}\n\`•\` Kullanıcı ID: \`${menu.clicker.member.id}\`\n\`•\` Kuruluş Tarihi: \`${moment(menu.clicker.member.user.createdTimestamp).locale("tr").format("LLL")} - (${moment(menu.clicker.member.user.createdTimestamp).locale("tr").fromNow()})\`\n\n**❯ Sunucu Bilgisi**\n\`•\` Sunucu İsmi: \`${nickname}\`\n\`•\` Katılım Tarihi: \`${moment(menu.clicker.member.joinedAt).locale("tr").format("LLL")} - (${moment(menu.clicker.member.joinedAt).locale("tr").fromNow()})\`\n\n\n\`•\` Rolleri (${rolleri.length}): ${rolleri.join(", ")}`)
    await menu.reply.edit(profilembed);}
  if (menu.values[0] === 'name') {
    await menu.reply.think(true);
    let teyitData = await teyit.findOne({
      userID: menu.clicker.member.id
  }) || {
      userName: []
  };
  let isimgeçmişiEmbed = new Discord.MessageEmbed()
  .setAuthor(menu.guild.name, menu.guild.iconURL({
      dynamic: true
  }))
  .setColor("RANDOM")
  .setDescription(`
**${menu.guild.name}** sunucusundaki isim geçmişin aşağıda belirtilmiştir..

${teyitData.userName.length <=0 ? `İsim Geçmişi Bulunamadı.` : `${menu.clicker.member} kişisinin toplamda ${teyitData.userName.length} isim kayıtı bulundu.`} 

${teyitData.userName.reverse().splice(0, 10).join("\n")}`)

menu.reply.edit(isimgeçmişiEmbed)
  }
  ...
  let data2 = await taglıData.find({
      authorID: menu.clicker.member.id,
      Durum: "puan"
  }) || [];
  ...
  Stat.findOne({
      ...
      }).map((x, index) => `${client.emojis.cache.find(x => x.name === "wex_noktacik")} ${menu.guild.channels.cache.get(x) ? menu.guild.channels.cache.get(x).name : "#kanal-bulunamadı"} \`${client.convertDuration(data.voiceCategory[x])}\``).join("\n");
      ...
      }).map((x, index) => `${client.emojis.cache.find(x => x.name === "wex_noktacik")} #${menu.guild.channels.cache.get(x) ? menu.guild.channels.cache.get(x).name : "#kanal-bulunamadı"} \`${client.convertDuration(data.voiceChannel[x])}\``).join("\n");
      ...
      }).map((x, index) => `${client.emojis.cache.find(x => x.name === "wex_noktacik")} #${menu.guild.channels.cache.get(x) ? menu.guild.channels.cache.get(x).name : "#kanal-bulunamadı"} \`${data.messageChannel[x]} mesaj\``).join("\n");
      let embed = new MessageEmbed()
          ...
          .setDescription(`${menu.clicker.member} adlı kullanıcının **${menu.guild.name}** sunucusundaki istatistikleri.\n`)
...
${client.emojis.cache.find(x => x.name === "wex_noktacik")} Toplam Kayıt: \`${teyit} kişi\`
${client.emojis.cache.find(x => x.name === "wex_noktacik")} Toplam Davet: \`${invite} kişi\`
${client.emojis.cache.find(x => x.name === "wex_noktacik")} Toplam Taglı: \`${taglı} kişi\`
${client.emojis.cache.find(x => x.name === "wex_noktacik")} Toplam Yetkili: \`${yetkili} kişi\`
`)

    
menu.reply.edit(embed)
  })}
  if (menu.values[0] === 'invite') {
    ...
    Database.findOne({
      ...
      if (!inviterData) {
        let embed = new MessageEmbed().setAuthor(menu.guild.name, menu.guild.iconURL({
          dynamic: true
      })).setColor("RANDOM")
          ...
          Database.find({
              guildID: menu.guild.id,
              inviterID: menu.clicker.member.id,
          }).sort().exec((err, inviterMembers) => {
              let dailyInvites = 0;
              if (inviterMembers.length) {
                  dailyInvites = inviterMembers.filter(x => menu.guild.members.cache.has(x.userID) && (Date.now() - menu.guild.members.cache.get(x.userID).joinedTimestamp) < 1000 * 60 * 60 * 24 * 7).length;
              };
              let embed2 = new MessageEmbed().setAuthor(menu.guild.name, menu.guild.iconURL({
                dynamic: true
            })).setColor("RANDOM")
              embed2.setDescription(`Toplam **${inviterData.regular+inviterData.bonus}** davete sahip! (**${inviterData.regular}** gerçek, **${inviterData.bonus}** bonus, **${inviterData.fake}** fake, **${dailyInvites}** haftalık)`);
           menu.reply.edit(embed2)
            });
      };
    })
  }
  if (menu.values[0] === 'cezai') {
    await menu.reply.think(true);
    let data = await ceza.find({userID: menu.clicker.member.id})
    if (data.length == 0) return menu.reply.edit("Bu kullanıcı daha önce bir ceza-i işleme tabi tutulmamış.")
    let embed2 = new MessageEmbed().setAuthor(menu.guild.name, menu.guild.iconURL({
      dynamic: true
  })).setColor("RANDOM")
    embed2.setDescription(`**${menu.guild.name}** sunucusundaki son **15** ceza-i işlem bilgilerin\n
${data.reverse().slice(0,10).map( (data, index) => `#${data.ID} **[${data.Ceza.toUpperCase()}]** (${data.Bitis == "null" ? "Aktif" : data.Bitis == "KALICI" ? "Aktif" : Date.now()>=data.Bitis ? "Kapalı" : "Aktif"}) \`${new Date(Number(data.Atilma)).toTurkishFormatDate()}\` tarihinde **${data.Sebep}** nedeniyle ${menu.guild.members.cache.has(data.Yetkili) ? menu.guild.members.cache.get(data.Yetkili).toString() : data.Yetkili} tarafından cezalandırıldı!`).join("\n")}`);
 menu.reply.edit(embed2)
}

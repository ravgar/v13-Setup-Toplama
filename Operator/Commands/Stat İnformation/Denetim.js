...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.member.permissions.has("ADMINISTRATOR") || durum) {
        ...
        message.channel.send(`${rol.map(target => `${target} - ${target.displayName}`).join("\n")}`, {
            code: "md",
            split: true
        }).then(async x => {
...
**Bilgiler**
Toplam: \`${(totalpoints+parseInt(ekPuan))}\`
Ek Puan: \`${ekPuan}\`

**Ses Bilgileri**
Public: \`${client.convertDuration(pubOda)} (${parseInt(pubOda/(1000 * 60 * 60 * 1) * pubPuan)} +Puan)\`
Streamer: \`${client.convertDuration(stream)} (${parseInt(stream/(1000 * 60 * 60 * 1) * streamPuan)} +Puan)\`
Oyun: \`${client.convertDuration(oyunodalar)} (${parseInt(oyunodalar/(1000 * 60 * 60 * 1) * oyunPuan)} +Puan)\`
Private: \`${client.convertDuration(secret)} (${parseInt(secret/(1000 * 60 * 60 * 1) * secretPuan)} +Puan)\`
Alone: \`${client.convertDuration(alone)} (${parseInt(alone/(1000 * 60 * 60 * 1) * alonePuan)} +Puan)\` ${kanallar.AloneKanallar.Id.length > 0 ? "" : " "}
Müzik: \`${client.convertDuration(music)} (${parseInt(music/(1000 * 60 * 60 * 1) * musicPuan)} +Puan)\` ${kanallar.Müzik.Id.length > 0 ? "" : " "}
Sleep: \`${client.convertDuration(sleeping)} (${parseInt(sleeping/(1000 * 60 * 60 * 1) * sleepPuan)} +Puan)\`
Terapi: \`${client.convertDuration(terapi)} (${parseInt(terapi/(1000 * 60 * 60 * 1) * terapipuan)} +Puan)\` ${kanallar.TerapiKanallar.Id.length > 0 ? "" : " "}
Sorun Çözme: \`${client.convertDuration(sçözme)} (${parseInt(sçözme/(1000 * 60 * 60 * 1) * sorunçözmepuan)} +Puan)\`
Kayıt Kanalları: \`${client.convertDuration(kayıt)} (${parseInt(kayıt/(1000 * 60 * 60 * 1) * kayitPuan)} +Puan)\`
Toplantı Kanalları: \`${client.convertDuration(meeting)} (${parseInt(meeting/(1000 * 60 * 60 * 1) * meetingPuan)} +Puan)\`
`)
            .addField("**Mesaj Bilgileri**", `Mesaj: \`${mesaj} (${(mesaj*mesajPuan).toFixed(0)} +Puan)\``)
            .addField("Diğer Bilgiler", ` 
Yetkili: \`${yetkili} (${yetkili*yetkiliPuan} +Puan)\`
Taglı: \`${taglı} (${taglı*taglıPuan} +Puan)\`
Davet: \`${invite} (${invite*invitePuan} +Puan)\`
Kayıt: \`${teyit} (${teyit*teyitPuan} +Puan)\`
`)

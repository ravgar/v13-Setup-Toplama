...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sunucuData.GKV.includes(message.author.id) || client.ayarlar.sahip.includes(message.author.id)) {
        let sec = args[0];
        ...
        if (["oluştur"].includes(sec)) {
            ...
            newData.save();
            message.channel.send({embeds: [new MessageEmbed().setColor("RANDOM").setFooter(client.ayarlar.footer).setTimestamp().setDescription(`\`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.
...
`)
 message.channel.send({embeds: [mesaj]})
        ...
        if (!sec) {
            ...
            message.channel.send({embeds: [new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setTimestamp().setFooter(client.ayarlar.footer).setDescription(`Aşağıdaki özel komutlar'ı daha detaylı bir şekilde görmek için aşağıdaki komutu yazınız.\n\`.özelkomut bak <komutAdı>\`\n\n${göster}`)]});
        };
    } else return;
};
exports.conf = {

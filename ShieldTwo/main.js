...
client.login(mainSettings.SHIELDTWO).catch(err => console.log("Token bozulmuş lütfen yeni bir token girmeyi dene"));
...
client.on("message", async (message) => {
  ...
  if (await message.member.permissions.has("ADMINISTRATOR")) return;
  let links = message.content.match(/(http[s]?:\/\/)(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-z]{2,6}?\b([-a-zA-Z0-9@:%_+.~#?&/=]*)/gi);
  const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk", ".gg/", ".gg"];

  if ((message.mentions.members.size >= 5 || message.mentions.roles.size >= 5 || message.mentions.channels.size >= 6)) {
...
const kufurEngel = new RegExp(["amcık","orospu","piç","sikerim","sikik","amına","pezevenk","orospu","evladı","göt","yarrak","o ç","siktir","bacını","karını","amk","sik","amq","anaskm","AMK","YARRAK","sıkerım"].join("|"), 'gi')
  ...
  if(client.ayarlar.sahip.includes(newMessage.member.id)) return;
  if(newMessage.member.permissions.has("ADMINISTRATOR")) return;
  if (newMessage.content.match(kufurEngel)) newMessage.delete().catch(err => {});
  ...
  if (newMessage.content.match(inviteEngel)) {
  ...
  if(reklamlar.some(word => newMessage.content.toLowerCase().includes(word))) newMessage.delete().catch(err => {})
...
function güvenli2(kisiID) {
  let uye = client.guilds.cache.get(sahip.sunucuId).members.cache.get(kisiID);
...
function Punish(kisiID, tur) {
  let MEMBER = client.guilds.cache.get(conf.sunucuId).members.cache.get(kisiID);
  if (!MEMBER) return;
